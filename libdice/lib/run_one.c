...
{
	while(
			((c_str < c_num_mem) & (c_str2 < c_num_mem2))
			&& c_str[rd_mem] == c_str2[rd_mem2] 
			&& (
				(__strcount32(c_str[rd_mem]) & 4)
				&& ae2f_expected(++c_str | ++c_str2 | 1)
			   )
	     ) {}

	ae2f_expected_but_else(c_str < c_num_mem)
		return 0xFFFFFFFF;

	ae2f_expected_but_else(c_str2 < c_num_mem2)
		return 0xFFFFFFFF;
...
{
	while(
			((c_str_0 < c_num_mem) & (c_str_1 < c_num_mem))
			&& c_str_0[rd_mem] == c_str_1[rd_mem]
			&& (
				(__strcount32(c_str_0[rd_mem]) & 4)
				&& ae2f_expected(++c_str_0 | ++c_str_1 | 1))
	     ) {}

	ae2f_expected_but_else(c_str_0 < c_num_mem)
		return 0xFFFFFFFF;

	ae2f_expected_but_else(c_str_1 < c_num_mem)
		return 0xFFFFFFFF;
